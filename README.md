...
4. **聊天（`/inbox`）** — WhatsApp Web 式三栏：左会话列表、中气泡线程、底栏作曲器。入站（Webhook 或「演示」注入）与人工发送、AI 待批准草稿都在同一窗口完成；高置信且打开自动回复才外发。手机号识别国家。  
5. **市场情报** — 适配器（seed / 手工 / 抓取桩）  
6. **三日复盘** — 批评与行动清单  
7. **设置** — 凭证、自动回复、毛利底线、目标、CSV 导入  

成功标准自测：新增一条知识库 → 聊天页「演示」注入询盘 → 同一会话里的草稿应引用该条目并可「批准发送」；产品页「排序推荐」应把低毛利 SKU 降权。
